namespace SnowyRiver.WPF.Modules.Splash;
public static class ViewNames
{
    private const string Prefix = "SnowyRiver.WPF.Modules.Splash.";
    public const string SplashView = Prefix + nameof(SplashView);
    public const string ProductInfoView = Prefix + nameof(ProductInfoView);
    public const string WelcomeView = Prefix + nameof(WelcomeView);
    public const string InitializationView = Prefix + nameof(InitializationView);
    public const string DbMigratorView = Prefix + nameof(DbMigratorView);
}
